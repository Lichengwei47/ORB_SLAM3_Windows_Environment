...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)//ok的时候不执行，输入新的影像
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);//输入第一帧影像和第二帧影像
        else
    ...
    mCurrentFrame.mNameFile = filename;
    mCurrentFrame.mnDataset = mnNumDataset;// ChangeDataset()，mnNumDataset++
...
void Tracking::Track()//system.cpp中new Tracking()赋值：mState==NO_IMAGES_YET；输入一帧影像后，mState变为NOT_INITIALIZED;System初始化的时候赋值，mbReset(false), mbResetActiveMap(false),mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
#ifdef SAVE_TIMES
    mTime_PreIntIMU = 0;
    mTime_PosePred = 0;
    ...
    if(mpLocalMapper->mbBadImu)
    {
        ...
        mpSystem->ResetActiveMap();//mMutexReset线程，IMU数据读取失败，mbResetActiveMap设置为True
        return;
    }

    Map* pCurrentMap = mpAtlas->GetCurrentMap();//地图Map在mpAtlas对象中

    if(mState!=NO_IMAGES_YET)//两帧影像进行初始化
    {
        if(mLastFrame.mTimeStamp>mCurrentFrame.mTimeStamp)
        {
            cerr << "ERROR: Frame with a timestamp older than previous frame detected!" << endl;
            unique_lock<mutex> lock(mMutexImuQueue);
            mlQueueImuData.clear();//与IMU相关
            CreateMapInAtlas();//跟踪失败，重新初始化，mState=NO_IMAGES_YET，mpInitializer设为空
            return;
        }
        else if(mCurrentFrame.mTimeStamp>mLastFrame.mTimeStamp+1.0)
        {
            cout << "id last: " << mLastFrame.mnId << "    id curr: " << mCurrentFrame.mnId << endl;
            if(mpAtlas->isInertial())//判断是否有IMU数据
            {
                ...
                if(mpAtlas->isImuInitialized())
                {
                    ...
                    {
                        mpSystem->ResetActiveMap();//mbResetActiveMap = true;跟踪失败？
                    }
                    else
                    {
                        CreateMapInAtlas();//跟踪失败，重新初始化，mState=NO_IMAGES_YET，mpInitializer设为空
                ...
                else
                {
                    ...
                    mpSystem->ResetActiveMap(); //mbResetActiveMap = true;帧不连续，跟踪失败
                }
            }
    ...
    if(mState==NO_IMAGES_YET)
    {
        mState = NOT_INITIALIZED;//输入第二张影像，mState改变
    }
    ...
    if(nCurMapChangeIndex>nMapChangeIndex)//不太懂？
    {
    ...
    if(mState==NOT_INITIALIZED)//两帧之间进行初始化
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO)
            StereoInitialization();
        else
        {
            MonocularInitialization();//单目初始化一些参数，恢复连续两帧的R、t，计算该Bow（用于快速回环检测），mState=OK;
        }
        ...
        {
            mnFirstFrameId = mCurrentFrame.mnId;//地图中没有点数据，将当前帧Id作为第一帧影像的Id
        }
    }
    else//两帧初始化失败
    {
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            mLastFrame = Frame(mCurrentFrame);
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());//mvbPrevMatched表示之前帧匹配成功的特征点
    ...
    else
    {
        ...
        {
            delete mpInitializer;//特征点小于100就删除了
        ...
        ORBmatcher matcher(0.9,true);//赋值给mfNNratio(0.9), mbCheckOrientation(true)
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Rcw,tcw,mvIniP3D,vbTriangulated))
        {
            ...
            CreateInitialMapMonocular();//位姿跟踪成功，mState=OK;将三角化后的点转换到第一帧的世界坐标系
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Update Connections
    pKFini->UpdateConnections();//更新关键帧之间的连接，在3D点和关键帧之间建立边
    ...
    pKFcur->SetPose(Tc2w);//设置第二帧的POS姿态
