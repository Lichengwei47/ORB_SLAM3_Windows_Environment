...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    vector<int> rotHist[HISTO_LENGTH];//创建了HISTO_LENGTH=30大小
    for(int i=0;i<HISTO_LENGTH;i++)
        rotHist[i].reserve(500);
    const float factor = 1.0f/HISTO_LENGTH;
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        if(level1>0)
            continue;//只取影像第一层的特征点

        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y, windowSize,level1,level1);//vIndices2为输入的(x,y)和F2中关键点距离<windowSize(100)的关键点的index值

        if(vIndices2.empty())
            continue;

        cv::Mat d1 = F1.mDescriptors.row(i1);//影像的BRIEF特征描述符
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            int dist = DescriptorDistance(d1,d2);//计算两特征点的HanMing距离
        ...
        if(bestDist<=TH_LOW)
        {
            if(bestDist<(float)bestDist2*mfNNratio)
            {
                ...
                if(mbCheckOrientation)
                {
                    ...
                    if(rot<0.0)
                        rot+=360.0f;
                    int bin = round(rot*factor);//检查角度是否大于阈值，factor=1/30
                    if(bin==HISTO_LENGTH)
                        bin=0;
                    assert(bin>=0 && bin<HISTO_LENGTH);
    ...
    if(mbCheckOrientation)
    {
        ...
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);//选择角度偏移数量最多的三组点（相当于剔除了错误匹配的点），以下代码使得所有同名特征点角度偏移一致
