...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    {
        unique_lock<mutex> lock(mMutexMode);
        if(mbActivateLocalizationMode)//仅仅相机追踪模式，初始化值是false
        {
            mpLocalMapper->RequestStop();//停止建立局部地图，关闭BA平差,mbStopped = True;mbStopRequested = True;
        ...
        if(mbDeactivateLocalizationMode)//slam建图与定位模式，初始化值是false
        {
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();//mbStopped = false;mbStopRequested = false;
            mbDeactivateLocalizationMode = false;
        }
    }
    ...
    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbReset)//初始化值是false,跟踪或者建图失败时候mbReset、mbResetActiveMap设置为True
        {
            mpTracker->Reset();
            mbReset = false;
            mbResetActiveMap = false;
        }
        else if(mbResetActiveMap)//初始化值是false
        {
