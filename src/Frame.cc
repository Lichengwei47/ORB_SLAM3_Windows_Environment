...
//static_cast<Pinhole*>(pCamera)->toK()李成伟8-9修改为GeometricCamera* pCamera=NULL
     ...
     mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    UndistortKeyPoints();//将该帧影像的特征点进行畸变矫正
    ...
    // This is done only for the first Frame (or after a change in the calibration)
    if(mbInitialComputations)//初始化
    {
        ComputeImageBounds(imGray);//计算图像边界左上、左下、右上、右下4个点畸变纠正后坐标
    ...
    AssignFeaturesToGrid();//计算各个特征点在哪个格子里面，保证特征点分布均匀
    ...
    mpMutexImu = new std::mutex();//这是在初始化什么？
}
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel, const bool bRight) const//bRight初始化false
{
